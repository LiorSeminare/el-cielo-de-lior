#!/usr/bin/env python3
# El Cielo de Lior - square 2400x2400 MOMENT sky (CIELO-MOM-01), story card variant
...
def parse_args(argv):
    p = {
        ...
        'story': 'LA NOCHE DE TU HISTORIA', 'out': 'cielo_momento_sample.png',
        'disp_hour': None,
    }
    extra_title = []
    for a in argv:
    ...
    if p['disp_hour'] is not None: p['disp_hour'] = float(p['disp_hour'])
...
center(120, P['title'], F_TITLE, INK)
MONTHS = ['enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre']
hh = int(P['disp_hour'] if P['disp_hour'] is not None else P['hour'])
mm = int(round(((P['disp_hour'] if P['disp_hour'] is not None else P['hour'])-hh)*60))
date_line = f"{P['day']} de {MONTHS[P['month']-1]} de {P['year']}  ·  {hh:02d}:{mm:02d} h  ·  {P['place']}"
center(240, date_line, F_SMALL, (110, 100, 85))

# story card panel (distinct from birth-sky strip)
CARD_X0, CARD_Y0, CARD_X1, CARD_Y1 = 360, CY + RMAX + 55, W - 360, CY + RMAX + 280
d.rounded_rectangle([CARD_X0, CARD_Y0, CARD_X1, CARD_Y1], radius=18,
                    fill=(252, 248, 238), outline=GOLD, width=3)
center(CARD_Y0 + 46, P['story'], F_CARDH, GOLD)
...
sky_line = ('Luna ' + phase_name + ' sobre el horizonte') if moon_drawn else 'Luna bajo el horizonte esta noche'
if visible_planets: sky_line += '   ·   ' + ', '.join(visible_planets) + ' visibles'
center(CARD_Y0 + 122, sky_line, F_CARD, (70, 64, 54))
center(CARD_Y0 + 190, 'una noche que merece su propio cielo', F_MICRO, (130, 120, 100))

center(2275, 'El Cielo de Lior  ·  Semilla Cósmica', F_MICRO, GOLD)
